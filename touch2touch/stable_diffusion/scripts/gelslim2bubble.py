...
def main():
    ...
    raw_data_path = os.path.join(outpath, "raw_data")
                ...
                for i in trange(len(data.datasets['test']), desc="Data"):
                    ...
                    for n in trange(opt.n_iter, desc="Sampling"):
                        for prompts in tqdm(prompts_lists, desc="within_datapoint"):
                            ...
                            # x_samples_ddim = (x_samples_ddim - torch.min(x_samples_ddim))/(torch.max(x_samples_ddim) - torch.min(x_samples_ddim))
                            ...
                            if not opt.skip_save:
                                ...
                                raw_data = {
                                    ...
                                    'input': (one_data['aux'][:,:3] * GELSLIM_STD[None,:,None,None] + GELSLIM_MEAN[None,:,None,None])/255.,
                                    'gt': (one_data['image'] + 1.) / 2. * (BUBBLES_MAX - BUBBLES_MIN) + BUBBLES_MIN,
                                    'prediction': (x_checked_image_torch + 1.) / 2. * (BUBBLES_MAX - BUBBLES_MIN) + BUBBLES_MIN,
                                }
                                torch.save(raw_data, os.path.join(raw_data_path, f"{one_data['index']}.pt"))
